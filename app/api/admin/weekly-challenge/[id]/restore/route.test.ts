...
describe('POST /api/admin/weekly-challenge/[id]/restore', () => {
  it('returns 200 when restoring a rejected puzzle', async () => {
    ...
    const req = new NextRequest('http://localhost/api/admin/weekly-challenge/wp-1/restore', { method: 'POST' })
    const res = await POST(req, { params: Promise.resolve({ id: 'wp-1' }) })
    expect(res.status).toBe(200)
  })

  it('returns 409 when the puzzle is not rejected', async () => {
    const { createClient, createAdminClient } = await import('@/lib/supabase/server')
